#!/usr/bin/env bash
# Agentic Classroom setup: configures the OpenClaw agent with YOUR OpenRouter key.
#
...
hr; echo " Agentic Classroom: setup"; hr
...
if [ -t 0 ] && [ -z "${CLASSROOM_NO_CHAT+x}" ] && agent_open; then
  ...
  trap - INT TERM HUP
  env -u OPENROUTER_API_KEY -u CLASSROOM_ROOT -u CLASSROOM_AUTOSTARTED \
