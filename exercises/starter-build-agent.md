...
- **This repository's Codespace works**, and so does any Python environment with a
  key for any model API that supports tool calling. For the Codespace, follow
  `README.md`: fork, add a key, create a Codespace. Student code reads the key
  from an environment variable (a Codespaces secret arrives as one), never from
  a file in the repository.
- The same Codespace runs `openclaw chat`, so students can work inside an agent
  while they build one, as the course's students did.
